...
def get_switch_id_serial_mapping():
    ...
    while i < num_lines:
        ...
        i += 1
    print("Switch ID to Serial mapping:")
...
def deploy_eem_script(name, event_pattern, actions):
    ...
    print(f'*** Successfully configured {name} EEM script on device! ***')
...
def main():
    ...
    if not switch_id_to_serial:
        print("Error: Unable to obtain the mapping of switch IDs to serial numbers. Please check the device output format and parsing function.")
        return
    ...
    if current_version_match and upgrade_version_match:
        ...
        if current_version == upgrade_version:
            print("*** Version matches, no upgrade needed ***")
        else:
            print("*** Version mismatch, initiating upgrade ***")
            ...
            deploy_eem_script(
                ...
                actions=[
                    ...
                    'action 1.3 wait 10',
                    f'action 2.0 regexp "Cisco IOS XE Software, Version ([0-9.]+)" $_cli_result version',
                    ...
                    'action 3.1 syslog msg "version mismatch, upgrade initiated"',
                    'action 4.0 end',
                    'action 4.1 end'
                ]
            )
        ...
        print("\n\n*** Current version matches the desired version, no upgrade required ***\n\n")
